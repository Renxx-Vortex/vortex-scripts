...
if not checkKey() then
    local KeyWindow = Fluent:CreateWindow({
        Title = "VORTEX UI",
        SubTitle = "Key System | Sniper Arena v6.0",
        TabWidth = 160,
    ...
    KeyTab:AddParagraph({
        Title = "Enter Key",
        Content = "Masukin key buat akses VORTEX UI"
    })
    ...
    KeyTab:AddButton({
        ...
        Callback = function()
            ...
            if saveKey(input) then
                KeyWindow:Destroy()
                loadstring(game:HttpGet("https://raw.githubusercontent.com/Renxx-Vortex/vortex-scripts/main/vortex-dxr-sniper-arena-v6.lua"))()
            else
                KeyTab:AddParagraph({
                    Title = "Error",
...
local Config = {
    -- Protection
    Bypass = false, AntiKick = false, AntiAFK = false, HideName = false, BoostFPS = false, LowPing = false, ShowFPS = false, ShowPing = false,
    -- Combat
    Aimbot = false, AimbotSmooth = 2, SilentAim = false, POVAimbot = false, POVSilentAim = false,
    TriggerBot = false, AutoFire = false, AutoShoot = false, AimPart = "Head",
    Hitbox = false, HitboxSize = 5, TeamCheck = false,
    -- Visuals
    ESP_Box = false, ESP_Line = false, ESP_Name = false, ESP_Health = false, ESP_Distance = false,
    ESP_Weapon = false, ESP_Skeleton = false, Chams = false, WeaponChams = false, Fullbright = false, NoFog = false, RadarHack = false,
    -- Weapon
    InfiniteAmmo = false, NoRecoil = false, NoSpread = false, FastReload = false, RapidFire = false,
    DamageMultiplier = false, DamageValue = 2, WeaponSize = 1,
    -- Movement
    GodMode = false, SpeedHack = false, SpeedValue = 30, JumpHack = false, JumpValue = 50,
    NoClip = false, Fly = false, FlySpeed = 50, InfiniteJump = false, TeleportToMouse = false,
}
...
Tabs.Combat:AddToggle("POVAimbot", { Title = "POV Aimbot", Default = false, Callback = function(v) Config.POVAimbot = v end })
Tabs.Combat:AddToggle("POVSilentAim", { Title = "POV Silent Aim", Default = false, Callback = function(v) Config.POVSilentAim = v end })
...
Tabs.Visuals:AddToggle("RadarHack", { Title = "Radar Hack", Default = false, Callback = function(v) Config.RadarHack = v end })
...
-- Notifikasi
Fluent:Notify({
    Title = "VORTEX UI v6.0",
    Content = "Script berhasil dimuat!",
    SubContent = "By DEEP & RENXX | 50 Features",
    Duration = 5
})

print("VORTEX UI v6.0 - Sniper Arena Loaded! (50 Features)")
print("By DEEP & RENXX")
